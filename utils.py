...
class Calibration:
    ...
    def img_to_rect(self, pts_img, depth_rect):
        ...
        :return: (N, 3)
        """
        pts_xy = (pts_img - self.c) * \
            np.repeat(depth_rect[:, np.newaxis], 2, axis=1) / self.f + self.t
...
class KittiObjectTemplate:
    ...
    def from_lidar_box(self, lidar_box: np.ndarray, calib: Calibration, cls_type=None, score=None):
...
class KittiObject3d(KittiObjectTemplate):
    ...
    def serialize(self):
        ...
        """
        return f"{self.cls_type} {self.truncation} {self.occlusion} {self.alpha} " \
...
class KittiTrack3d(KittiObjectTemplate):
    def __init__(self, line: str = None, object3d: KittiObject3d = None, sample_id: int = None, tracking_id: int = None, img_hw=None):
        if line is None:
            if object3d is None:
                super().__init__()
            else:
            ...
            self.box2d = np.array((float(label[6]), float(label[7]), float(
                label[8]), float(label[9])))
            self.h = float(label[10])
            self.w = float(label[11])
            self.l = float(label[12])
            self.loc = np.array((float(label[13]), float(
                label[14]), float(label[15])), dtype=np.float32)
    ...
    def serialize(self):
        return f"{self.sample_id} {self.tracking_id} {self.cls_type} {self.truncation} {self.occlusion} {self.alpha} " \
            f"{' '.join(self.box2d.astype(str).tolist())} {self.h} {self.w} {self.l} " \
            f"{' '.join(self.loc.astype(str).tolist())} {self.ry} {self.tracking_score}\n"
    ...
    if as_dict:
        if track:
            ...
            objects = {
                ...
                'dimensions': np.array([(o.l, o.h, o.w) for o in objects]).reshape(-1, 3),
...
def map_tracks_by_frames(seq_tracks: List[KittiTrack3d]) -> Dict[int, List[KittiTrack3d]]:
    out = {}
...
def rotate_points_along_z(points: np.ndarray, angles: np.ndarray):
    ...
    return np.concatenate((np.matmul(points[:, :, 0:3], rot_matrix), points[:, :, 3:]), axis=-1)
...
def rotate_points_bev(points: np.ndarray, angles: np.ndarray):
    ...
    return np.concatenate((np.matmul(points[:, :, 0:2], rot_matrix), points[:, :, 2:]), axis=-1)
...
def points_inside_boxes(boxes: np.ndarray, points: np.ndarray):
    ...
    masks = np.all(np.less_equal(
        np.abs(local_points) * 2, boxes[:, np.newaxis, 3:6]), axis=2)
    return masks
...
def get_global_boxes_from_lidar(boxes_lidar: np.ndarray, pose: Tuple[np.ndarray, np.ndarray]):
    ...
    return np.concatenate((loc_global, boxes_lidar[:, 3:6], boxes_lidar[:, 6:7] + yaw), axis=1)


def get_lidar_boxes_from_global(boxes_global: np.ndarray, pose: Tuple[np.ndarray, np.ndarray]):
    ...
    yaw = np.arctan2(R[1, 0], R[0, 0])
    return np.concatenate((loc_lidar, boxes_global[:, 3:6], boxes_global[:, 6:7] - yaw), axis=1)


def visualize_trajectories(trajectories: List[np.ndarray], other_boxes: np.ndarray = None):
    fig, ax = plt.subplots()
    ax.axis('equal')
    for i, boxes in enumerate(trajectories):
        ...
        x = -boxes[:, 1]
        y = boxes[:, 0]
        ax.quiver(x[:-1], y[:-1], x[1:]-x[:-1], y[1:]-y[:-1], units='xy',
                  angles='xy', scale=1, color=f'C{i}', headaxislength=4)
...
def crop_points_from_boxes(points: np.ndarray, boxes: np.ndarray, front_only=True) -> List[np.ndarray]:
    """
    Args:
...
def compute_iou_2d(bboxes1: np.ndarray, bboxes2: np.ndarray, do_ioa=False) -> np.ndarray:
...
def get_boxes2d_from_instance_map(instance_map: np.ndarray, ignored_inst_ids=None):
    ...
    for inst_id in inst_ids:
        ...
        boxes2d.append(np.array((np.min(col_inds), np.min(
            row_inds), np.max(col_inds), np.max(row_inds))))
    return np.stack(boxes2d) if len(boxes2d) > 0 else np.empty((0, 4))


def convert_trajectory_keys_from_track_id_to_frame(trajectories: Dict[int, Tuple[List[np.ndarray], List[KittiTrack3d]]]):
    frame_dict = {}
