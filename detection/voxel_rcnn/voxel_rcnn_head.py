...
class VoxelRCNNHead(nn.Module):
    def __init__(self, backbone_channels, model_cfg, point_cloud_range, voxel_size, **kwargs):
        ...
        for k in range(0, self.model_cfg['SHARED_FC'].__len__()):
            shared_fc_list.extend([
                nn.Linear(
                    pre_channel, self.model_cfg['SHARED_FC'][k], bias=False),
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        batch_size = batch_dict['batch_size']
        with_vf_transform = batch_dict.get(
            'with_voxel_feature_transform', False)
        ...
        roi_grid_batch_cnt = rois.new_zeros(
            batch_size).int().fill_(roi_grid_coords.shape[1])
