...
from .pointnet2_stack import voxel_pool_modules
from .utils import (class_agnostic_nms, generate_voxel2pinds,
                    get_voxel_centers, rotate_points_bev)


class VoxelRCNNHead(nn.Module):
    def __init__(
        self, backbone_channels, model_cfg, point_cloud_range, voxel_size, **kwargs
    ):
        super().__init__()
        ...
        self.grid_size = self.pool_cfg["GRID_SIZE"]
        pre_channel = self.grid_size**3 * c_out
        ...
        for k in range(0, self.model_cfg["REG_FC"].__len__()):
            ...
            if (
                k != self.model_cfg["REG_FC"].__len__() - 1
                and self.model_cfg["DP_RATIO"] > 0
            ):
        ...
        self.reg_pred_layer = nn.Linear(
            pre_channel, self.box_coder.code_size, bias=True
        )

    @torch.no_grad()
    def proposal_layer(self, batch_dict: dict, nms_config: dict):
        """
        Args:
            batch_dict:
                batch_size:
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        roi_grid_batch_cnt = (
            rois.new_zeros(batch_size).int().fill_(roi_grid_coords.shape[1])
        )
    ...
    def get_global_grid_points_of_roi(self, rois, grid_size):
        ...
        global_roi_grid_points = rotate_points_bev(
    ...
    def generate_predicted_boxes(self, batch_size, rois, cls_preds, box_preds):
        ...
        batch_box_preds = rotate_points_bev(
    ...
    def forward(self, batch_dict):
        ...
        self.proposal_layer(batch_dict, nms_config=self.model_cfg["NMS_CONFIG"])
