...
def weighted_box_fusion(box_scores, box_preds, box_labels, fusion_thresh=0.55, score_thresh=None, n_models=1):
    if score_thresh is not None:
        scores_mask = (box_scores >= score_thresh)
        box_scores = box_scores[scores_mask]
        box_preds = box_preds[scores_mask]
    ...
    if num_boxes == 0:
        ...
        for i in range(1, num_boxes):
            bev_ious = iou3d_nms_utils.boxes_iou_bev(
                torch.stack(fused_boxes), box_preds[i:i+1]).view(-1)
            ...
            if bev_ious[max_idx] <= fusion_thresh:
                ...
                fused_boxes[max_idx] = torch.sum(
                    cluster_scores[max_idx][:, None].repeat(1, 7) * cluster_boxes[max_idx], dim=0,
                ) / torch.sum(cluster_scores[max_idx])
        
        for k in range(len(fused_scores)):
            fused_scores[k] = torch.mean(cluster_scores[k]) * min(n_models, len(cluster_scores[k])) / n_models
