        ...
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                thresholds = get_thresholds(
                    thresholdss, total_num_valid_gt, num_sample_pts=N_SAMPLE_PTS
                )
...
def get_official_eval_result(gt_annos, dt_annos, current_classes) -> Tuple[str, float]:
    min_overlaps = np.array(
        [[[0.7, 0.5, 0.5], [0.7, 0.5, 0.5], [0.7, 0.5, 0.5]]]  # bbox  # bev
    )  # 3d
    # car, ped, cyc
    ...
    (
        ...
        mAP3d_R40,
        mAPaos_R40,
    ) = do_eval(gt_annos, dt_annos, current_classes, min_overlaps, compute_aos)
    ...
    for j, curcls in enumerate(current_classes):
        ...
        result += print_str(
            (
                f"bbox AP:{mAPbbox_R40[j, 0, 0]:.4f}, "
