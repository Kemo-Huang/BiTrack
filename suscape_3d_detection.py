...
from datasets.suscape_dataset import (SuscapeDataset, collate_fn,
                                      format_suscape_detection_results)
...
def model_inference(
    ...
    for seq in range(21):
        ...
        if test_time_aug_cfgs is None:
            ...
            for cam_batch_dict in pbar:
                res = {}
                for cam, batch_dict in cam_batch_dict.items():
                    for key, value in batch_dict.items():
                        if key not in ['frame_id', 'batch_size']:
                            batch_dict[key] = torch.from_numpy(
                                value).float().cuda()
                    with torch.no_grad():
                        batch_dict = model.forward(batch_dict)
                    pred_dicts = model.post_processing(batch_dict)
                    res[cam] = format_suscape_detection_results(
                        pred_dicts,
            ...
            for batch_dict in pbar:
                ...
                torch.save(pred_dict, seq_middle_dir /
                           f"{'_'.join(batch_dict['frame_id'])}.pt")
            ...
            for aug_name, aug_params in test_time_aug_cfgs.items():
                for aug_idx, aug_param in enumerate(aug_params):
                    ...
                    seq_middle_dir = output_dir / 'middle_data' / \
                        aug_name / str(aug_idx) / seq
                    seq_middle_dir.mkdir(exist_ok=True, parents=True)
...
def tta_fusion(root_path, output_dir: Path, model, class_names, test_time_aug_cfgs: dict):
    for seq in range(21):
        seq = str(seq).zfill(4)
...
def main():
    ...
    if tta:
        tta_fusion(root_dir, output_dir, model,
                   class_names, test_time_aug_cfgs)
