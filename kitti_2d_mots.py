    ...
    for seq in seq_paths:
        ...
        for img_file in pbar:
            ...
            if args.load_seg:
                ...
                img = img.cuda()
                seg_input = F.pad(
                    img, (0, 0, 1248-w, 384-h), padding_mode='edge')
            ...
            if args.embedding:
                ...
                if len(xyxys) > 0:
                    ...
                    if args.load_emb:
                        embeds = np.load(seq_emb_out_dir /
                                         f'{img_file.stem}.npy')
                    else:
                        with torch.no_grad():
                            embeds = embed_net.forward(
                                fg_points, bg_points, xyxys)
                        embeds = embeds.cpu().numpy()
                        np.save(seq_emb_out_dir /
                                f'{img_file.stem}.npy', embeds)
                else:
                    embeds = np.array([])
                    masks = np.array([])

                if args.tracking:
                    trackHelper.tracking(seq.name, int(
                        img_file.stem), embeds, masks)
