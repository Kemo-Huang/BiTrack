...
class BranchedERFNet(nn.Module):
    ...
    def cluster(self, prediction: torch.Tensor, n_sigma=1, threshold=0.5, min_pixel=128) -> torch.Tensor:
        ...
        seed_map = torch.sigmoid(
            prediction[2 + n_sigma:2 + n_sigma + 1])  # 1 x h x w
        ...
        if num_pixel > min_pixel:
            spatial_emb_masked = spatial_emb[mask.expand_as(
                spatial_emb)].view(2, num_pixel)
