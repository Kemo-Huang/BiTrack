...
def write_kitti_detection_results(pred_dicts, class_names, frame_ids, calibrations, img_hw_dict, output_dir: Path):
    ...
    for batch_idx, box_dict in enumerate(pred_dicts):
        ...
        lines = []
        if num_samples > 0:
            calib = calibrations if isinstance(
                calibrations, Calibration) else calibrations[batch_idx]
            names = np.array(class_names)[pred_labels - 1]

            for i in range(num_samples):
                obj = KittiObject3d(img_hw=img_hw_dict[str(frame_id).zfill(6)]).from_lidar_box(
                    pred_boxes[i], calib, names[i], pred_scores[i])
                line = obj.serialize()
                lines.append(line)
...
class KittiDataset(Dataset):
    def __init__(
        ...
        self.sample_ids = [f.stem for f in (
            self.root_path / 'velodyne' / self.seq).iterdir()]
    ...
    def get_lidar(self, idx):
        return np.fromfile(self.root_path / 'velodyne' / self.seq / f'{self.sample_ids[idx]}.bin', dtype=np.float32).reshape(-1, 4)
    ...
    def __getitem__(self, idx):
        ...
        proj, mask = get_frustum_points(
            points[:, :3], self.calib, h, w, kitti=True)
        points = points[mask]
