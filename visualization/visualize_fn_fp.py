...
def main():
    ...
    trk_out_dir = Path(f'output/{args.tag}/data')
    gt_dir = Path('data/tracking/label_02')
    calib_dir = Path('data/tracking/calib')
    oxts_dir = Path('data/tracking/oxts')
    img_dir = Path('data/tracking/image_02')
    det_out_dir = Path('data/tracking/voxel_rcnn_output_0.1')

    use_poses = False
    use_dets = False  # use detection results instead of tracking results
    show_fn = False
    ...
    with open('data/tracking/evaluate_tracking.seqmap.training') as f:
        lines = f.readlines()
    ...
    for seq, frames in seq_frame_dict.items():
        ...
        gt = 0
        if use_dets:
            seq_out_tracks = {int(frame.stem): get_objects_from_label(frame) for frame in (det_out_dir / seq).iterdir()}
        else:
        ...
        for frame in range(int(start_frame), int(end_frame)):
            ...
            if (show_fn and len(bboxes3) - matches > 0):
