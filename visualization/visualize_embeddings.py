            ...
            if embed_file.exists():
                ...
                if cur_lidar_dir.exists():
                    ...
                    for lidar_file in cur_lidar_dir.iterdir():
                        ...
                        inside_points.append(np.fromfile(lidar_file, dtype=np.float32).reshape(-1, 4)[:, :3])
                    instance_img = np.array(Image.open(instance_file))
                    instance_ids, _ = get_instance_ids_of_boxes_hungarian(instance_img, inside_points, calib, min_n_pts)
                    embeds = np.load(embed_file)
                    
                    good_box_mask = instance_ids > 0
                    all_embeds.append(embeds[instance_ids[good_box_mask] - 1])
                    corres_gt_ids.append(np.array(gt_ids, dtype=int)[good_box_mask] + 1000 * i)
