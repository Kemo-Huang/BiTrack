...
def get_label_annos(label_dir, sample_ids=None):
    ...
    if sample_ids is None:
        ...
        label_files = [(label_dir / f'{str(sample_id).zfill(6)}.txt') for sample_id in sample_ids]
    return [get_objects_from_label(label_file, as_dict=True) for label_file in label_files]
...
def evaluate(label_path,
    ...
    res_str = get_official_eval_result(
        gt_annos, dt_annos, classes)
    return res_str
...
def main():
    ...
    print(evaluate(args.label_path, args.result_path,
          args.label_split, args.classes))
