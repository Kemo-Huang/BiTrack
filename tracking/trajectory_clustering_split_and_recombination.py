...
def get_overlaps_of_trajectories(
    ...
    for i, a_id in enumerate(a_ids):
        ...
        assert all(a_frames[idx] < a_frames[idx + 1]
                   for idx in range(len(a_frames) - 1)), str(a_frames)

        for j, b_id in enumerate(b_ids):
            _, b_objs = b_trajectories[b_id]
            b_frames = [b_obj.sample_id for b_obj in b_objs]
            # check frames are sorted
            assert all(b_frames[idx] < b_frames[idx + 1]
                       for idx in range(len(b_frames) - 1)), str(b_frames)
...
def get_clusters(overlap_arr) -> List[Tuple[List[int]]]:
    ...
    while a_idx_set or b_idx_set:
        ...
        if a_idx_set:
            ...
            cur_inds = [j for j in range(
                m) if overlap_arr[cur_idx][j] and j in b_idx_set]
            is_a = True
        else:
            cur_idx = b_idx_set.pop()
            cluster_b.append(cur_idx)
            cur_inds = [i for i in range(
                n) if overlap_arr[i][cur_idx] and i in a_idx_set]
            is_a = False
        while len(cur_inds) > 0:
            ...
            for x in cur_inds:
                if is_a:
                    b_idx_set.remove(x)
                    cluster_b.append(x)
                    next_inds += [i for i in range(n)
                                  if overlap_arr[i][x] and i in a_idx_set]
                else:
                    a_idx_set.remove(x)
                    cluster_a.append(x)
                    next_inds += [j for j in range(m)
                                  if overlap_arr[x][j] and j in b_idx_set]
...
def merge_distinct(common_inds_dict, ids, trajectories, cluster_distinct_inds, all_boxes, all_objs, all_frames):
...
def merge_forward_backward_trajectories(
    ...
    a_ids, b_ids, overlap_pair_inds = get_overlaps_of_trajectories(
        a_trajectories, b_trajectories)
    is_overlapped = [[len(overlap_pair_inds[i][j]) > 0 for j in range(
        len(b_ids))] for i in range(len(a_ids))]
    ...
    for cluster in clusters:
        ...
        if merge_common_only:
            ...
            if len(set(cluster_common_frames)) == len(cluster_common_frames):
                if len(cluster_common_objs) > 0:
                    ...
                    merged_trajectories[cur_id] = (
                        cluster_common_boxes, cluster_common_objs)
                    cur_id += 1
                continue
            ...
            for a_idx in cluster_a_inds:
                a_boxes, a_objs = a_trajectories[a_ids[a_idx]]
                a_distinct_inds = [x for x in range(
                    len(a_objs)) if x not in cluster_a_common_inds[a_idx]]
            ...
            for b_idx in cluster_b_inds:
                ...
                b_distinct_inds = [x for x in range(
                    len(b_objs)) if x not in cluster_b_common_inds[b_idx]]
                cluster_b_distinct_boxes = [b_boxes[x] for x in b_distinct_inds]
                cluster_b_distinct_objs = [b_objs[x] for x in b_distinct_inds]

            cluster_all_boxes = cluster_common_boxes + cluster_a_distinct_boxes + cluster_b_distinct_boxes
            cluster_all_objs = cluster_common_objs + cluster_a_distinct_objs + cluster_b_distinct_objs
        ...
        for a_idx in cluster_a_inds:
            for b_idx in cluster_b_inds:
                ...
                for a_common_idx, b_common_idx in overlap_pair_inds[a_idx][b_idx]:
                    ...
                    b_common_idx = b_len - 1 - b_common_idx  # reverse indices for backward scores
                    if last_a_idx is None:
                        cur_tracklet = [
                            a_idx, b_idx, a_common_idx, b_common_idx, 1, [cur_frame]]
                    else:
                        if a_common_idx == last_a_idx + 1 and b_common_idx == last_b_idx - 1:
                            # consecutive
        ...
        all_choices = before_choices + before_bad_choices + \
            after_choices + after_bad_choices
        connections = [[None, None] for _ in range(
            len(common_tracklets))]  # (before, after) forward
        ...
        for x in np.argsort(all_scores)[::-1]:
            ...
            if x < 2 * len(common_tracklets):  # before
                x %= len(common_tracklets)
                if connections[x][0] is None and connections[cur_choice][1] is None and tracklets_frames[cur_choice].isdisjoint(tracklets_frames[x]):
                ...
                x %= len(common_tracklets)
                if connections[x][1] is None and connections[cur_choice][0] is None and tracklets_frames[cur_choice].isdisjoint(tracklets_frames[x]):
        ...
        for merged_tracklet_inds in merged_tracklet_inds_list:
            ...
            for idx in merged_tracklet_inds:
                a_idx, b_idx, a_last_idx, b_last_idx, track_len, cur_frames = common_tracklets[
                    idx]
                a_boxes, a_objs = a_trajectories[a_ids[a_idx]]
                b_boxes, b_objs = b_trajectories[b_ids[b_idx]]
        ...
        if visualize_contradictions:
            visualize_trajectories(
                [x[0] for x in cur_merged_trajectories.values()])
