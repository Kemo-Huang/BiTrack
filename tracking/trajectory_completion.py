...
from utils import (Calibration, KittiTrack3d, sort_trajectory_by_frame,
...
def kf_predict_tail_boxes(frames: List[int], boxes: List[np.ndarray], tail_length) -> List[np.ndarray]:
...
def linear_interpolation(
    ...
    for frame, candidates in candidates_dict.items():
        ...
        cand_boxes = np.array([x[1] for x in candidates], dtype=np.float32).reshape(-1, 7)
        ...
        if ignore_thresh > 0:
            ...
            if frame in existing_boxes:
                ex_boxes = np.array(existing_boxes[frame], dtype=np.float32).reshape(-1, 7)
        ...
        if nms_thresh != 0:
            ...
            while len(order) > 0:
                ...
                order = order[:-1]
                dis_aff_matrix = compute_ncd_matrix(picked_box[np.newaxis], cand_boxes[order])
    ...
    # for trk_id, trajectory in trajectories.items():
    #     interp_nums[trk_id] = len(trajectory[0]) - interp_nums[trk_id]
    # print('interp nums:', list(interp_nums.values()))
    if visualize:
        visualize_trajectories([trajectories[tid][0] for tid in vis_ids])
