...
def box_size_weighted_mean(
    ...
    for trk_id, trajectory in trajectories.items():
        ...
        boxes[:, 3:6] = np.sum(np.tile(scores[:, np.newaxis], (1, 3)) * boxes[:, 3:6], axis=0) / np.sum(scores)
