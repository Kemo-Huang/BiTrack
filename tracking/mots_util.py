...
def export_tracking_result_in_kitti_format(
    tag, tracks, add_masks, model_str, out_folder="", start_time_at_1=False
):
    if out_folder == "":
            ...
            for track in tracks_t:
                if add_masks:
                    print(
                        ...
                        track.class_,
                        *track.mask["size"],
                        track.mask["counts"].decode(encoding="UTF-8"),
                        file=f
                    )
                else:
...
class TrackHelper(object):
    def __init__(
        self,
        save_dir,
        t_car=0.8165986526897969,
        t_person=0.47985540892434836,
        alive_car=5,
        alive_person=30,
        car=True,
        mask_iou=False,
        mask_iou_scale_person=0.54,
        mask_iou_scale_car=0.74,
        cosine=False,
    ):
    ...
    def tracking(self, subfolder, frameCount, embeds, masks):
        self.current_video = (
            subfolder if self.current_video == None else self.current_video
        )
        ...
        last_reids = np.concatenate(
            [el.embed[np.newaxis] for el in self.active_tracks], axis=0
        )
        curr_reids = embeds
        ...
        reid_dists = cdist(
            curr_reids, last_reids, "euclidean" if not self.cosine else "cosine"
        )
        ...
        if self.mask_iou:
            ...
            masks_t = [
                maskUtils.encode(np.asfortranarray(v.astype(np.uint8))) for v in masks
            ]
        ...
        for row, column in indexes:
            ...
            current_inst = newElem(
                frameCount,
                self.active_tracks[column].track_id,
                embed,
                mask,
