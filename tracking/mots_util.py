...
TrackElement = namedtuple(
    "TrackElement", ["t", "track_id", "class_id", "mask", "embed", "points"])
        ...
        for t, tracks_t in enumerate(tracks, start):  # TODO this works?
            for track in tracks_t:
                if add_masks:
                    ...
                    print(t, track.track_id, class_str, -1, -1, -1, *
                          track.box, -1, -1, -1, -1, -1, -1, -1, track.score, file=f)
...
class TrackHelper(object):
    ...
    def __init__(self, save_dir, t_car=0.8165986526897969, t_person=0.47985540892434836, alive_car=5, alive_person=30, car=True,
    ...
    def export_last_video(self):
        ...
        with open(out_filename, "w") as f:
            for track in self.all_tracks:
                print(track.t, track.track_id, track.class_id, *
                      track.mask['size'], track.mask['counts'].decode(encoding='UTF-8'), file=f)
    ...
    def tracking(self, subfolder, frameCount, embeds, masks):
        ...
        if len(self.all_tracks) == 0 or len(self.active_tracks) == 0:
            ...
            for i in range(n):
                embed = embeds[i]
                mask = masks[i]
                current_inst = newElem(
                    frameCount, self.next_inst_id, embed, mask, class_id=self.class_id)
        ...
        last_reids = np.concatenate([el.embed[np.newaxis]
                                    for el in self.active_tracks], axis=0)
        curr_reids = embeds
        asso_sim = np.zeros((n, len(self.active_tracks)))

        detections_assigned = np.zeros(len(embeds)).astype(bool)
        reid_dists = cdist(curr_reids, last_reids,
                           "euclidean" if not self.cosine else 'cosine')
        asso_sim += self.reid_euclidean_scale * \
            (self.reid_euclidean_offset - reid_dists)

        if self.mask_iou:
            # consider add mask iou
            masks_t = [maskUtils.encode(np.asfortranarray(
                v.astype(np.uint8))) for v in masks]
            masks_tm1 = [v.mask for v in self.active_tracks]
            mask_ious = maskUtils.iou(
                masks_t, masks_tm1, [False] * len(masks_tm1))
        ...
        for row, column in indexes:
            ...
            mask = masks[row]
            current_inst = newElem(
                frameCount, self.active_tracks[column].track_id, embed, mask, class_id=self.class_id)
            self.all_tracks.append(current_inst)
            self.active_tracks[column] = current_inst
            detections_assigned[row] = True

        # new inst for unassigned
        detections_assigned_Inds = np.nonzero(
            detections_assigned == False)[0].tolist()
