...
def get_detection_data(
    ...
    # Converts to tracks
    objs = [
        KittiTrack3d(object3d=obj, sample_id=int(
            det3d_file.stem), tracking_id=None)
        for obj in objs
    ]
    ...
    if lidar_dir is None:
        if det2d_file is None and seg_file is None:
            ...
            good_detections = Detections(
                boxes, objs, None, None, None)
            bad_detections_3d = Detections(
                np.empty(0), np.empty(0), None, None, None)
            ...
            if det2d_file is not None:
                ...
                corr_2d_inds, similarity = get_corr_2d_inds_hungarian(
                    calib, img_hw, proj_boxes, 'box', boxes2d, 'box', min_corr_pts, min_corr_iou
                )
            else:
                # 3d box + 2d mask
                corr_2d_inds, similarity = get_corr_2d_inds_hungarian(
                    calib, img_hw, proj_boxes, 'box', instance_map, 'mask', min_corr_pts, min_corr_iou, all_instance_ids
                )
    else:
        inside_points = [np.fromfile(
        lidar_dir / f'{i}.bin', np.float32).reshape(-1, 4)[:, :3] for i in range(len(boxes))]
        ...
        if det2d_file is None and seg_file is None:
            ...
            good_detections = Detections(
                boxes[good_3d_mask], objs[good_3d_mask], similarity[good_3d_mask], None, None)
            bad_detections_3d = Detections(
            ...
            if det2d_file is not None:
                # 3d point + 2d box
                corr_2d_inds, similarity = get_corr_2d_inds_hungarian(
                    calib, img_hw, inside_points, 'point', boxes2d, 'box', min_corr_pts, min_corr_iou
                )
            elif seg_file is not None:
                # 3d point + 2d mask
                corr_2d_inds, similarity = get_corr_2d_inds_hungarian(
                    calib, img_hw, inside_points, 'point', instance_map, 'mask', min_corr_pts, min_corr_iou, all_instance_ids
                )
    ...
    if num_2d != -1:
        ...
        if embed_dir is None:
            ...
            good_embeds = [np.load(embed_dir / f'{x}.npy').reshape(-1) for x in good_corr_2d_inds]
            bad_embeds = [np.load(embed_dir / f'{x}.npy').reshape(-1) for x in bad_2d_inds]
        ...
        good_detections = Detections(
            boxes[good_3d_mask], objs[good_3d_mask], similarity[good_3d_mask], good_embeds, good_corr_2d_inds)
        bad_detections_3d = Detections(
            boxes[bad_3d_mask], objs[bad_3d_mask], similarity[bad_3d_mask], None, None)
        bad_detections_2d = Detections(
            None, None, None, bad_embeds, bad_2d_inds)
    
    accept_mask = np.array([obj.tracking_score >= recover_score_thresh for obj in bad_detections_3d.objs], dtype=bool)
    accept_detections_3d = Detections(bad_detections_3d.boxes3d[accept_mask], bad_detections_3d.objs[accept_mask], None, None, None)
    if len(accept_detections_3d) > 0:
        good_detections.append_3d(accept_detections_3d)
        bad_detections_3d = Detections(bad_detections_3d.boxes3d[~accept_mask], bad_detections_3d.objs[~accept_mask], None if bad_detections_3d.similarity is None else bad_detections_3d.similarity[~accept_mask], None, None)
...
def get_corr_2d_inds_hungarian(
    ...
    if type_3d == 'point':
        ...
        for i, points in enumerate(data_3d):  # for each 3d object
            ...
            valid_mask = (depth > 0) & (pts2d[:, 0] >= 0) & (pts2d[:, 0] <= img_hw[1] - 1) & (
                pts2d[:, 1] >= 0) & (pts2d[:, 1] <= img_hw[0] - 1)
            ...
            if type_2d == 'mask':
                ...
                valid_mask = (ext_pts2d[..., 0] >= data_2d[:, 0]) & (ext_pts2d[..., 0] <= data_2d[:, 2]) & (
                    ext_pts2d[..., 1] >= data_2d[:, 1]) & (ext_pts2d[..., 1] <= data_2d[:, 3])
        ...
        if type_2d == 'box':
            ...
            for i, box in enumerate(data_3d):  # for each 3d object
                for j, mask_inds in enumerate(all_mask_inds): # for each 2d instance
                    valid_mask = (mask_inds[0] >= box[0]) & (mask_inds[1] >= box[1]) & (mask_inds[0] <= box[2]) & (mask_inds[1] <= box[3])
...
def get_2d_box_inds_of_3d_boxes_hungarian(
    ...
    for i, points in enumerate(inside_points):  # for each 3d box
        ...
        # projects points to image
        pts2d, depth = calib.lidar_to_img(points)
        valid_mask = (depth > 0) & (pts2d[:, 0] >= 0) & (pts2d[:, 0] < img_hw[1]) & (
            pts2d[:, 1] >= 0) & (pts2d[:, 1] < img_hw[0])
        pts2d = pts2d[valid_mask]
        total_pc_cnts[i] = len(pts2d)

        # computes the number of inside pixels for each 3d box and 2d box
        ext_pts2d = pts2d[:, np.newaxis, :]
        valid_mask = (ext_pts2d[..., 0] >= boxes2d[:, 1]) & (ext_pts2d[..., 0] <= boxes2d[:, 3]) & (
            ext_pts2d[..., 1] >= boxes2d[:, 0]) & (ext_pts2d[..., 1] <= boxes2d[:, 2])
    ...
    pc_ious = np.divide(final_pc_counts, total_pc_cnts,
                        out=np.zeros(N, dtype=float), where=total_pc_cnts != 0)
