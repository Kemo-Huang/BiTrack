...
class Track:
    ...
    def __init__(self, box: np.ndarray, obj, offline=True, embed=None, momentum=0.9, p=10, q=2, ang_vel=True, vel_reinit=True):
        self.id = Track.global_cur_id
        Track.global_cur_id += 1
        self.filter = CVKalmanFilter(box, p=p, q=q, ang_vel=ang_vel, vel_reinit=vel_reinit)
        self.embed = embed
        self.misses = 0
        self.hits = 0
