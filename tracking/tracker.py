...
def compute_3d_iou_matrix_shapely(boxes_a, boxes_b):
    ...
    polys_a = [Polygon(np.concatenate((corners, corners[0:1]), axis=0))
               for corners in boxes_to_corners_bev(boxes_a)]
    polys_b = [Polygon(np.concatenate((corners, corners[0:1]), axis=0))
...
def compute_center_dis_matrix(det_boxes, pred_boxes):
    center_dis_matrix = np.linalg.norm(
        det_boxes[:, np.newaxis, :3] - pred_boxes[np.newaxis, :, :3],
        ord=2, axis=-1
    )
...
class Tracker:
    ...
    def optimize(
        self, aff_matrix, aff_thresh=0,
        det_scores=None, trk_scores=None,
        ...
        if self.matcher.algorithm == 'HA' or det_scores is None:
            ...
            return self.matcher.match(
                aff_matrix, aff_thresh,
                post_valid_mask=post_valid_mask,
                unused=unused
            )
    ...
    def aff_match(self, det_boxes, pred_boxes, det_embeds=None,
                  match_matrix=None, entry_vec=None,
                  exit_vec=None, false_det_vec=None, false_trk_vec=None,
                  det_scores=None, trk_scores=None,
                  det_valid_mask=None, trk_valid_mask=None,
                  ):
        ...
        if det_valid_mask is None or trk_valid_mask is None:
            results = self.optimize(
                aff_matrix, self.aff_thresh,
                det_scores, trk_scores,
                post_valid_mask,
                unused=app_matrix
            )
            return results
    ...
    def predict(self, num_passed_frames=1) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if len(self.tracks) > 0:
            ...
            pred_boxes = np.stack([trk.predict(num_passed_frames)
    ...
    def associate(self, pred_boxes: np.ndarray, detections: Detections = None):
        ...
        if num_dets > 0 and num_trks > 0:
            ...
            match_matrix, entry_vec, exit_vec, false_det_vec, false_trk_vec = self.aff_match(
                det_boxes, pred_boxes, det_embeds,
                match_matrix, entry_vec, exit_vec, false_det_vec, false_trk_vec,
                det_scores, trk_scores,
                det_valid_mask,
                trk_valid_mask,
            )
    ...
    def update(self, matched, entry_dets, exit_trks, false_trks, detections: Detections):
        if detections is None:
            det_boxes = []
            det_embeds = None
            objs = None
        ...
        for d, t in matched:
            ...
            self.tracks[t].update(
                det_boxes[d], obj=None if objs is None else objs[d],
        ...
        for i in entry_dets:
            self.tracks.append(Track(
                det_boxes[i], obj=None if objs is None else objs[i],
    ...
    def track_management(self) -> Tuple[List[Track], List[Track]]:
        ...
        for trk in reversed(self.tracks):
            ...
            elif trk.misses == 0 and (self.frame_counter <= self.t_hit or trk.hits >= self.t_hit):
